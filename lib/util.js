...
function convertData(data) {
  var seriesData = [];
  var count = 0;
  var props = data.propertiesCount;
 ...
 * @returns {Array}
 */
function createTitle(path, atTime) {
  var title = 'StyleStats Test Result';
  var uri = path.replace(/http(s)?\:\/\//, '');
  var time = moment(atTime).format('MM/DD/YY HH:mm:ss');
  title = title + ' | ' + uri + ' - ' + time;
  return title;
}
